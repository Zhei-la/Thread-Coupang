...
- 제품명·브랜드명 직접 언급 금지.`;
  ...
  if (hasCommentLure) {
    fixedToneInstr += `\n\n[댓글유도형 적용 - 매우 중요]
- 글 본문은 일상적인 이야기를 하다가 "근데 여기서 한 가지가 중요한데" 처럼 궁금증만 던지고 그냥 끊어.
- "댓글에", "댓글 봐", "나머지는" 같은 직접 유도 표현 절대 금지.
- 그냥 말하다가 자연스럽게 뚝 끊기는 느낌. 독자가 알아서 댓글 찾게 만들 것.
- 나쁜 예시(절대 금지): "나머지는 댓글에 적어줄게", "어떻게 했는지 댓글에"
- 좋은 예시: "근데 여기서 한 가지가 진짜 중요한데" → 그냥 끝.`;
  }
...
- 게시글 텍스트만. 설명·주석·따옴표 없이.`;
  ...
  if (type === 'comment') {
    ...
    const commentLureInstr = hasCommentLure
      ? '\n\n[댓글 작성 규칙 - 중요]\n이 댓글은 본문에서 던진 궁금증을 실제로 해결해주는 내용이야.\n당연한 말 절대 금지 (예: "간장 많이 넣으면 짜요" 이런 거 금지).\n진짜 아는 사람만 아는 구체적인 팁을 알려줘.\n예를 들어 간장계란밥이면: 어떤 간장인지, 파나 참기름 같이 넣은 거 있는지, 계란 올리는 타이밍 같은 구체적인 정보.\n센스있게, 짧고 핵심만, 반말로, 이모지 없이, 마침표 없이.'
      : '';
